...
import { TMTransportUtils } from '../network/network';

import * as Types from './chain.types';

export type * from './chain.types';
...
export class EtcdSharedChain {
    config: Types.EtcdSharedChainConfiguration;
    client: Etcd3;
    redisClient: Redis.RedisClientType | null = null;
    current: EtcdSharedChainItem | null = null;
    separateStorageDecoder: (x: any) => any;

    static defaultEtcdSharedChainConfiguration(commonPrefix = "", useDate = true): Types.EtcdSharedChainConfiguration {
    ...
    constructor(config: Types.EtcdSharedChainConfiguration, separateStorageDecoder?: (x: any) => any) {
        this.config = config;
        this.client = new Etcd3(config.etcd3Options);
        if (config.duplicateFromRedis || config.automaticallyDuplicateToRedis) {
            this.redisClient = Redis.createClient({ url: `redis://${this.config.redisServerAddr}` });
        }
        this.current = null;
            ...
            if (this.config.automaticallyDuplicateToRedis) {
                if (this.config.redisTTLSeconds > 0) {
                    await this.redisClient!.set(
                        ...
                        , { 'EX': this.config.redisTTLSeconds }
                    );
                } else {
                    await this.redisClient!.set(
                        this.config.chainPrefix + ":" + thisID
...
export class RedisSharedChain {
    ...
    client: Redis.RedisClientType | null = null;
    current: RedisSharedChainItem | null = null;
    decoder: ((x: any) => any);
    ...
    constructor(config: RedisSharedChainConfiguration, decoder?: ((x: any) => any)) {
        this.config = config;
        this.client = Redis.createClient({ url: `redis://${this.config.redisServerAddr}` });
        this.current = null;
        if (decoder) {
    ...
    async start(defaultData: any) {
        ...
        let redisReply = await this.client!.eval(luaStr, { keys: [headKeyStr] });
        if (redisReply != null) {
            this.current = {
                id: this.config.headKey
                , data: defaultData
                , nextID: redisReply.toString()
            };
        }
    }
    ...
    async tryAppend(newID: string, newData: any): Promise<boolean> {
        ...
        let redisReply = await this.client!.eval(luaStr, { keys: [currentChainKey, newDataKey, newChainKey], arguments: [newID, cbor.encode(newData)] });
...
export type SharedChainSpec = ["etcd", Types.EtcdSharedChainConfiguration] | ["redis", RedisSharedChainConfiguration];

export class SharedChainUtils {
    static parseChainLocator(locatorStr: string): SharedChainSpec | null {
        if (locatorStr.startsWith('etcd://')) {
            let parsedParts = TMTransportUtils.parseConnectionLocator(locatorStr.substr('etcd://'.length));
            let conf: Types.EtcdSharedChainConfiguration = {
