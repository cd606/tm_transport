...
namespace dev { namespace cd606 { namespace tm { namespace transport { namespace redis {
    ...
    public:
        ...
        static std::future<basic::ByteDataWithTopic> fetchFirstUpdateAndDisconnect(Env *env, ConnectionLocator const &locator, std::string const &topic, std::optional<WireToUserHook> hook = std::nullopt) {
            ...
            bool done = false;
            *id = env->redis_addSubscriptionClient(
                locator
                , topic 
                , [env, ret, id, done](basic::ByteDataWithTopic &&d) mutable {
                    if (!done) {
                        ...
                        std::thread([env, ret, id, d = std::move(d)]() {
                            ...
                            env->redis_removeSubscriptionClient(*id);
                            try {
                    ...
                    if (!done) {
                        ...
                        if (tRes) {
                            if (!predicate || predicate(t)) {
                                ...
                                if (env) {
                                    env->log(infra::LogLevel::Info, "[RedisImporterExporter::fetchTypedFirstUpdateAndDisconnect] received data");
                                }
