...
namespace dev { namespace cd606 { namespace tm { namespace transport { namespace grpc_interop {
    ...
    namespace detect_experimental {
        template <class T>
        struct TrueType : std::true_type {};

        template <class T>
        static auto hasMarkMethodCallbackHelper(int) -> TrueType<decltype(std::declval<T>().MarkMethodCallback(0, nullptr))>;
        template <class T>
        static auto hasMarkMethodCallbackHelper(long) -> std::false_type;

        class GrpcServiceExperimentalChecker {
        public:
            static constexpr bool hasMarkMethodCallback = decltype(hasMarkMethodCallbackHelper<grpc::Service>(0))::value;
        };
    }
    ...
    private:
        ...
        )
        {
            ...
            public:
                    ...
                    , reactors_(), mutex_()
                {
                    ...
                    if constexpr (!detect_experimental::GrpcServiceExperimentalChecker::hasMarkMethodCallback) {
        ...
        )
        {
            ...
            private:
                ...
                std::mutex mutex_;
            public:
                LocalService(Env *env, GrpcServiceInfo const &serviceInfo, std::function<void(typename M::template Key<std::tuple<std::string,Req>> &&)> const &triggerFunc) 
                    : grpc::Service(), env_(env), triggerFunc_(triggerFunc) 
                    , serviceInfoStr_(grpcServiceInfoAsEndPointString(serviceInfo))
                    , reactors_(), mutex_()
                {
                ...
                grpc::ServerWriteReactor<Resp> *serviceFunc(grpc::CallbackServerContext *ctx, const Req *req) {
                    ...
                    if (auth->IsPeerAuthenticated()) {
                        for (auto const &item : auth->GetPeerIdentity()) {
                            if (item.length() > 0) {
                                identity = std::string {item.data(), item.length()};
                                break;
                            }
                        }
                    }
                    ...
                    triggerFunc_(typename M::template Key<std::tuple<std::string,Req>> {
                        id
                        , {std::move(identity), *req}
                    });
                    return iter->second.get();
                }
                void handleResp(typename M::template InnerData<typename M::template KeyedData<std::tuple<std::string,Req>, Resp>> &&resp) {
                    std::lock_guard<std::mutex> _(mutex_);
                    auto iter = reactors_.find(resp.timedData.value.key.id());
                    if (iter != reactors_.end()) {
                        iter->second->pushReply(
            ...
            >([service](typename M::template InnerData<typename M::template KeyedData<std::tuple<std::string,Req>, Resp>> &&resp) {
                service->handleResp(std::move(resp));
            });
            r.registerExporter(wrapperItemsNamePrefix+"/exporter", exporter);
            toBeWrapped(r, r.importItem(importer), r.exporterAsSink(exporter));
