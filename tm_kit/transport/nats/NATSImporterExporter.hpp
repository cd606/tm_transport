#ifndef TM_KIT_TRANSPORT_NATS_NATS_IMPORTER_EXPORTER_HPP_
#define TM_KIT_TRANSPORT_NATS_NATS_IMPORTER_EXPORTER_HPP_
    ...
    public:
        ...
        static std::shared_ptr<typename M::template Importer<basic::ByteDataWithTopic>> createImporter(ConnectionLocator const &locator, std::string const &topic="*", std::optional<WireToUserHook> wireToUserHook=std::nullopt) {
            ...
            public:
                ...
                virtual void start(Env *env) override final {
                    ...
                    if (!client_) {
                        client_ = env->nats_addSubscriptionClient(
                            locator_
                            , topic_
                ...
                virtual void control(Env *env, std::string const &command, std::vector<std::string> const &params) override final {
                    std::thread th([this,env,command]() {
                        if (command == "stop") {
                            std::lock_guard<std::mutex> _(mutex_);
                            if (client_) {
                                env->nats_removeSubscriptionClient(*client_);
                                client_ = std::nullopt;
                            }
        ...
        static std::shared_ptr<typename M::template Exporter<basic::ByteDataWithTopic>> createExporter(ConnectionLocator const &locator, std::optional<UserToWireHook> userToWireHook=std::nullopt, std::string const &heartbeatName = "") {
            ...
            public:
                ...
                virtual void start(Env *env) override final {
                    env_ = env;
                    publisher_ = env->nats_getPublisher(locator_, userToWireHook_);
                    ...
                    >) {
                        static_cast<HeartbeatAndAlertComponent *>(env)->addBroadcastChannel(
                            heartbeatName_
                            , std::string("nats://")+locator_.toSerializationFormat()
                        );
                    }
                }
                ...
                , [env, ret, id, done](basic::ByteDataWithTopic &&d) mutable {
                    if (!done) {
                        ...
                        std::thread([env, ret, id, d = std::move(d)]() {
                            ...
                            env->nats_removeSubscriptionClient(*id);
                            try {
